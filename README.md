...
- **Téléchargements nommés** (`product_characteristics`, [ADR-009](.claude/decisions/009-telechargements-nommes.md)) : le bouton global d'un champ fichier (`file-link.html.twig`) affiche « Télécharger (FORMAT POIDS) », ce qui ne convient plus dès qu'un bloc porte **plusieurs** documents (deux boutons identiques). Le helper **`_drive_matic_document_downloads()`** (`drive_matic.theme`) parcourt les champs fichier demandés et renvoie une liste `label/url/format/size` prête à afficher ; le SDC rend le bouton lui-même (les slots `notice`/`documentation` ont donc disparu au profit d'une prop `downloads`). Le **libellé est éditorial** : c'est la **description du fichier** saisie en back-office (`description_field: true` sur `field_file_notice`/`field_file_doc`, texte d'aide dédié), avec repli sur le **nom du fichier sans extension** ; format et poids restent calculés. Comme le fichier n'est pas rendu par son formatter, ses **cache tags sont réattachés** explicitement. Réutilisable pour F6 Documentations.
...
- **Slideshow** (ADR-008) : **Swiper vendorisé** (`vendor/swiper/swiper-bundle.min.{js,css}`, global `Swiper` + a11y, **self-contained**/RGPD). Libs `drive_matic/swiper` (assets) + `drive_matic/slideshow` (behavior `driveMaticSlideshow` : init Swiper **si ≥ 2** `.swiper-slide`, sinon repli liste ; `prefers-reduced-motion` → `speed: 0` ; flèches masquées tant que non initialisé ; empilement sans JS). Global `Swiper` déclaré dans `eslint.config.mjs`. Premier usage : `history` (réutilisé par V4 home + V5 produit).
  ...
  - **Flèches hors de la piste** : `jumbo_home` les superpose au visuel (dans le `.swiper`), `history` les place dans l'en-tête du bloc. Le behavior cherche flèches **et** pagination dans le conteneur du composant, donc les deux placements marchent sans JS dédié ; quand elles sont hors de la piste, l'état « démarré » se lit via `:has()` (cf. ADR-008).
