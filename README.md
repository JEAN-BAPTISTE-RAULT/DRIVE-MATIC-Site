...
- **Téléchargements nommés** (`product_characteristics`) : le bouton global d'un champ fichier (`file-link.html.twig`) affiche « Télécharger (FORMAT POIDS) », ce qui ne convient plus dès qu'un bloc porte **plusieurs** documents (deux boutons identiques). Le helper **`_drive_matic_document_downloads()`** (`drive_matic.theme`) prend une carte `nom de champ → libellé front` et renvoie une liste `label/url/format/size` prête à afficher ; le SDC rend le bouton lui-même (les slots `notice`/`documentation` ont donc disparu au profit d'une prop `downloads`). Le libellé front est porté par le **thème**, pas par le libellé de champ (administratif : « Notice technique (à télécharger) »). Comme le fichier n'est pas rendu par son formatter, ses **cache tags sont réattachés** explicitement. Réutilisable pour F6 Documentations.
