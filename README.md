...
**Titre affiché vs libellé d'administration** ([ADR-011](.claude/decisions/011-titre-affiche-et-alias.md)) : sur les **11 types publics hors `homepage`**, le `title` du node est un **libellé d'administration** (relabellisé par un `core.base_field_override.node.<bundle>.title`) et le champ **`field_title`** (obligatoire) porte le titre affiché. `drive_matic_preprocess_page_title()` le substitue au titre de route sur `entity.node.canonical` uniquement — le champ est **masqué dans le view display** pour éviter le titre en double. Tout rendu hors page canonique (carte, teaser, ligne de Vue) lit `field_title`, jamais `label`.

**Alias** : motif Pathauto `/[node:field_title]` sur tous les types publics ; **exception** `news` → `/actualites/[node:field_title]` (la page `all_news` vivant à `/actualites`, le segment parent est un chemin valide, ce qui donne un vrai fil d'Ariane à 3 niveaux). Aucun motif sur `homepage` ni sur les fragments.
...
**Langue** : site **monolingue français** (décision #6), `language` + `locale` installés, français seule langue et langue par défaut. Traductions d'interface importées par `drush locale:update` (15 309 chaînes, 27 projets) ; les traductions de **configuration** sont versionnées dans `config/sync/language/fr/` (193 fichiers). Fil d'Ariane : Easy Breadcrumb lit `field_title` via `alternative_title_field` — sans quoi il affiche le **titre administratif**, car il se sert du titre de route et non de la substitution posée par le thème ; la capitalisation automatique est désactivée.
...
**Balises meta** ([ADR-010](.claude/decisions/010-metatags.md)) : remplissage **automatique** par les défauts Metatag + champ de **surcharge** `field_meta_tags` (widget `metatag_firehose` en barre latérale du formulaire, masqué au rendu) sur tous les nodes publics **sauf `legals`** — **à ajouter à chaque nouveau node public**. Deux étages de défauts : le défaut global `node` reste sur `title: [node:title] | [site:name]` (il sert de repli aux bundles sans `field_title` — `homepage`, fragments : un token vide donnerait « | DRIVE-MATIC »), et **chaque bundle public a son propre défaut** `metatag.metatag_defaults.node__<bundle>` en `[node:field_title] | [site:name]`. `legals`, qui n'a ni body ni champ de surcharge, garde malgré tout un défaut de bundle limité au **titre** — sans quoi sa balise `<title>` afficherait le libellé d'administration. ⚠️ Sur `<front>`, Metatag applique son défaut spécial **`front`** qui **remplace** les défauts `node`/`node__homepage` : le mapping y est donc recopié, en gardant `canonical_url: [site:url]` (la canonique de l'accueil doit rester la racine, pas `/node/<id>`). La surcharge par le champ du node s'applique malgré tout sur l'accueil. Détail et raison dans `docs/active/content-types/model.md`.
