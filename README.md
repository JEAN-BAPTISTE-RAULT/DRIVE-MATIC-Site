...
- **Thème admin** : **Gin** (toolbar horizontale, `gin.settings.classic_toolbar: horizontal` + `gin_toolbar` + `admin_toolbar`). ⚠️ **Le module core `navigation` doit rester désinstallé** : dès qu'il est actif, `gin_toolbar` pose `gin--core-navigation` et **sort avant** d'appliquer la classe horizontale — la barre latérale verticale du cœur gagne et le réglage de Gin devient silencieusement inopérant.
