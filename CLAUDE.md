...
- **Deux onglets horizontaux** (`field_group`, format `tabs` / `direction: horizontal`, groupes `group_tabs` > `group_general` + `group_content`) :
  - **« Informations generales »** : `title`, `field_title`, `path`, `field_meta_tags`, `status` — dans cet ordre.
...
- **Titre affiche vs libelle d'administration** ([ADR-011](.claude/decisions/011-titre-affiche-et-alias.md)) : le `title` du node est un **libelle d'administration** (relabellise « Titre administratif » par un `core.base_field_override.node.<bundle>.title`) ; le titre **affiche** est porte par **`field_title`** (obligatoire), substitue au titre de route par `drive_matic_preprocess_page_title()` et **masque dans le view display**. Tout **nouveau type public** doit donc recevoir : `field_title`, son `base_field_override` de `title`, un defaut `metatag.metatag_defaults.node__<bundle>` en `[node:field_title] | [site:name]`, et son motif Pathauto. Exceptions : `homepage` (titre porte par un paragraphe, pas d'alias) et les fragments (`question`, `document`, `brand` — pas de page publique). Tout rendu hors page canonique (carte, teaser, ligne de vue) lit `field_title`, jamais `label`.
- **Alias d'URL** : motif Pathauto `/[node:field_title]` pour tout node public ; `news` fait exception avec `/actualites/[node:field_title]`. Un nouveau type public a besoin de son propre `pathauto.pattern.node_<bundle>`.
...
- **Nouveau type « fragment »** (contenu reutilisable sans page publique) : pas de `field_title`, pas de `field_meta_tags`, pas de motif Pathauto, **pas** de reglage sitemap, et un `rabbit_hole.behavior_settings.node_type_<bundle>` en `access_denied`. Verifier le 403 en anonyme sur `/node/<id>`, jamais seulement l'absence de lien.
