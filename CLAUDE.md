...
- ⚠️ **Une entite content custom (`#[ContentEntityType]`) n'expose AUCUNE donnee a Views par defaut — pas meme ses champs de base — tant que son attribut ne declare pas explicitement `handlers: ['views_data' => \Drupal\views\EntityViewsData::class]`.** Sans ce handler, `\Drupal::service('views.views_data')->get('<table>')` ne renvoie que `<table>_bulk_form` (rien d'autre) : une vue construite sur cette base table s'enregistre et s'importe SANS erreur de schema, mais plante en 500 a l'affichage (`SELECT` genere vide, `ORDER BY "unknown"`) des qu'un champ/tri est reellement resolu — aucun message n'indique la cause reelle (handler manquant), seule l'erreur SQL brute apparait dans les logs. Ajouter le handler (+ la dependance `drupal:views` dans le `.info.yml` si le module ne l'a pas deja) resout tout d'un coup : champs, tri par colonne ET filtres redeviennent normalement configurables. A verifier des la premiere vue construite sur une entite custom du projet (`equipment_price`/`delivery_address`/`quote_configuration` ne l'ont pas non plus aujourd'hui). Cas vecu sur `quote` (listing admin `/admin/content/devis`).
- ⚠️ **Un champ `list_string`/`list_integer` porte par une entite content custom (base field, pas un champ Field API) n'obtient PAS le plugin de filtre `list_field` via le handler Views generique (`EntityViewsData`) — seulement `string`, quelle que soit sa liste de valeurs autorisees.** Un filtre expose avec `plugin_id: list_field` pose sur ce type de champ est silencieusement ignore par Views (pas d'erreur, le filtre n'apparait simplement jamais dans le formulaire expose). Pour obtenir malgre tout un menu deroulant avec les libelles humains plutot qu'un champ texte libre exigeant la valeur machine, utiliser un **filtre groupe** sur `plugin_id: string` (`is_grouped: true`, un `group_info.group_items` par valeur autorisee, meme mecanisme que le filtre "Statut de publication" du view `content` du coeur, qui groupe un simple champ boolean). Cas vecu sur le filtre Statut du listing `/admin/content/devis`.
- ⚠️ **Etendre `\Drupal\Core\Controller\ControllerBase` avec une propriete promue par le constructeur portant le meme nom qu'une propriete deja fournie par la classe (`entityTypeManager`, `formBuilder`, `currentUser`, `moduleHandler`, `configFactory`...) leve un fatal PHP au chargement de la classe** (« Type of ...::$xxx must not be defined ») : `ControllerBase` declare ces proprietes **sans type**, avec un getter paresseux (`$this->entityTypeManager()`), et PHP interdit de leur ajouter un type dans une sous-classe. Verifier la liste des proprietes de `ControllerBase` avant d'injecter quoi que ce soit ; utiliser la methode heritee, ou un autre nom de propriete. **N'existe pas sur `FormBase`** (verifie : ne declare aucune de ces proprietes) — piege specifique a `ControllerBase`. Cas vecu sur `QuoteDetailController` (1er `Controller` du projet).
- ⚠️ **Le favicon "automatique" de Drupal (`system.theme.global`, `favicon.use_default: true`) ne cherche `favicon.ico` QU'A LA RACINE du theme actif — jamais dans un sous-dossier — et retombe silencieusement sur l'icone generique du cœur (`core/misc/favicon.ico`) si absent, sans aucune erreur ni log** (`ThemeSettingsProvider::buildThemeSettings()`, `file_exists($themeObject->getPath() . '/favicon.ico')`). Deposer un jeu de favicons dans `images/favicon/` (ou tout autre sous-dossier, coherent avec le rangement habituel des assets statiques du theme) laisse donc ce mecanisme continuer a servir l'icone du cœur, meme avec des balises `<link>` explicites ajoutees par ailleurs dans `html.html.twig` — les deux `<link rel="icon">` coexistent sans erreur, ce qui masque le probleme (le navigateur choisit souvent la bonne icone quand meme, cf. tailles/ordre). Toujours dupliquer (ou ne placer que) `favicon.ico` a la racine du theme si ce mecanisme reste actif, plutot que de desactiver `toggle.favicon` globalement (ce reglage n'est pas scope par theme, une desactivation globale retire aussi le favicon du theme admin Gin). Cas vecu sur `drive_matic` (ajout du favicon, 2026-09-03).
- ⚠️ **Une cellule de `#type: table` dont la valeur est un render array (ex. `Link::toRenderable()`) doit etre enveloppee dans `['data' => ...]` — un tableau nu sans cle `data` est interprete par `template_preprocess_table()` comme des ATTRIBUTS HTML bruts pour la cellule, jamais comme du contenu a rendre.** Sans ce wrapper, l'erreur ne se voit qu'au rendu final (pas au build du render array) : `RuntimeException: Unexpected type for $value (...)` leve dans `Attribute::__toString()`, des que l'une des cles du render array (ex. `#url`, un objet `Url`) est interpretee comme une valeur d'attribut. Toujours envelopper une cellule de tableau non-scalaire, meme un simple lien : `['data' => $renderable]`.
...
- ⚠️ **Retirer un item d'une ligne de grille CSS en auto-placement (`.dm-form-row-start` sur l'item suivant) ne redonne PAS sa largeur aux items restants de cette ligne : chaque item garde exactement sa largeur/son `span` declare (1 colonne par defaut), la colonne liberee reste simplement vide.** Pour qu'un champ elargisse reellement (ex. un libelle trop long pour tenir sur 1 colonne), il faut lui poser explicitement `dm-form-span-2` (ou l'equivalent `grid-column-end: span N`) — deplacer un voisin sur la ligne suivante ne suffit pas. Cas vecu sur le webform `account_request` : un 1er jet mettait Siret/TVA/Raison sociale sur une meme ligne (3 colonnes), puis a tente de faire tenir le libelle « Numero de TVA intracommunautaire » en deplacant seulement Raison sociale sur la ligne suivante — le champ TVA restait a 1/3 de largeur, libelle toujours coupe en 2 lignes ; corrige en ajoutant `dm-form-span-2` sur TVA (qui a alors naturellement pousse Raison sociale a la ligne suivante, sans meme avoir besoin d'un `dm-form-row-start` explicite dessus).
...
**E-mails via `hook_mail()` + Mailer Policy (hors Webform)**

Mecanisme distinct du precedent : un module custom sans formulaire Webform (ex. `drivematic_configurator`, ADR-036) envoie un e-mail transactionnel via `hook_mail()` classique (`\Drupal\Core\Mail\MailManagerInterface::mail()`), dont le contenu final est pilote par une config `mailer_policy.mailer_policy.<module>.<key>.yml` (`/admin/config/system/mailer-policy`) plutot que par un handler Webform. Reprend le meme gabarit HTML/CSS inline que les e-mails webform ci-dessus.

- ⚠️ **Ne jamais poser de corps dans `$message['body']` a l'interieur d'un `hook_mail()` prevu pour etre entierement pilote par `mailer_policy`.** `BodyEmailAdjuster` (module `mailer_policy`) sauvegarde tout corps HTML DEJA present sur l'e-mail dans une variable Twig nommee `body`, avant de poser le nouveau corps — mais le gabarit d'habillage par defaut (`symfony_mailer/templates/email-wrap.html.twig`) lit LUI AUSSI une variable nommee `body` pour afficher le contenu final : le texte de repli de `hook_mail()` ecrase alors silencieusement le gabarit configure (aucune erreur, le sujet et le HTML autour restent corrects, seul le corps est le mauvais). Poser le texte de repli sur `$message['plain']` (alimente `$email->setTextBody()`, l'alternative texte du MIME, jamais lue par cet adjuster) et laisser `$message['body']` vide.
- ⚠️ **`t()` leve un `TypeError` (jamais un simple champ vide) des qu'un placeholder `@xxx` recoit `NULL`** — ce qui arrive des qu'un champ optionnel du compte/de l'entite est vide (ex. `field_phone`, un complement d'adresse). Caster explicitement chaque valeur en `(string)` avant de la passer en placeholder `t()` dans `hook_mail()`, et chaque valeur retournee par `hook_tokens()` — jamais la valeur de champ nue. Pas un cas rare : se produit des le premier compte reel dont un champ optionnel n'est pas renseigne.
- Jetons dedies au module (`hook_token_info()`/`hook_tokens()`, type de jeton propre, ex. `[quote:reference]`) plutot que de compter sur le module contrib `token` pour exposer generiquement les champs d'une entite custom (comportement non verifie pour ce cas) — l'API Token utilisee ici est celle du **core**, aucune dependance supplementaire.

**PDF (Dompdf)**

Genere un document PDF a partir d'un gabarit Twig autonome (HTML/CSS inline, meme convention que les e-mails ci-dessus) : voir `QuotePdfGenerator` (module `drivematic_configurator`, ADR-041) pour le pattern de reference (rendu Twig hors contexte de page via `renderer->renderInIsolation()`, toutes les valeurs pre-formatees en PHP, jamais de logique dans le gabarit).

- ⚠️ **Une image raster PNG encodee en 16 bits par canal (ou plus largement, de trop basse resolution native) peut produire un artefact visuel une fois rendue par Dompdf en qualite print (~300 DPI)** — un residu de trait, invisible a la taille d'affichage web habituelle (ex. un logo d'e-mail affiche a 30-70px), devient manifeste une fois agrandi pour un rendu print. Un reencodage en 8 bits (GD) attenue le symptome sans forcement l'eliminer si la cause reelle est la resolution native insuffisante. **Reflexe** : verifier tout asset raster integre a un PDF en rendant reellement le document et en zoomant (pas seulement a l'oeil sur une capture a taille normale — meme piege que la rigueur maquette, CLAUDE.md plus haut) ; preferer un **SVG** des qu'un original vectoriel existe (aucune limite de resolution, quelle que soit la taille de sortie).
- `Dompdf::setIsRemoteEnabled(false)` (defaut) : aucune ressource distante n'est chargee. Tout asset (logo, police) doit etre integre en **data URI base64** dans le gabarit — jamais un chemin fichier ou une URL, qui echoueraient silencieusement selon la config `Options::setChroot()`.
...
- **Committer des secrets** : `settings.php`/`settings.local.php`, cles API, identifiants de base ou tokens ne sont jamais versionnes (utiliser variables d'environnement / fichiers ignores). **Meme regle pour un champ sensible d'une entite de configuration versionnee** (ex. mot de passe SMTP d'un `mailer_transport`, cle secrete reCAPTCHA) : committer l'entite avec ce champ **vide**, et poser la vraie valeur uniquement via une surcharge `$config['<config_name>'][...] = '...';` dans le `settings.php` de l'environnement concerne (jamais synchronise) — `ConfigFactory` applique cette surcharge **au-dessus** du stockage a la lecture, elle survit donc a tout `drush config:import` ulterieur sans qu'aucun script de deploiement n'ait besoin de la connaitre. Ne jamais creer l'entite directement en base sur un serveur (drush/UI) sans l'exporter en config versionnee : le prochain `config:import` la supprimerait, n'ayant aucune trace d'elle dans `config/sync/`.
