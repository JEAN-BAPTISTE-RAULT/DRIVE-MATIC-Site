...
- Les panneaux sans carte (Drive Matic, Assistance) sont centres horizontalement dans le panneau ; les panneaux a cartes (Auto-ecole, Vehicule PMR) restent cales a la gouttiere gauche.
- Le lien « Espace partenaire », **en anonyme** (deconnecte), n'est jamais souligne (repos, survol, focus) — verifier en session reellement anonyme, pas juste en admin/partenaire connecte (la version connectee est un `<button>`, jamais concernee par ce piege).
...
**Mise en oeuvre (a rejouer) — panneaux sans carte centres et soulignage anonyme corrige, le 2026-09-10** : (1) `Drive Matic`/`Assistance` (panneaux sans `&__cards`) sont desormais centres dans `.site-header__panel-inner` (`justify-content: center`), au lieu de rester cales a la gouttiere gauche avec un grand vide a droite ; `Auto-ecole`/`Vehicule PMR` (a cartes) inchanges. (2) Le lien « Espace partenaire » anonyme (`<a href="/user/login">`, distinct du `<button>` connecte) n'est plus souligne — bug reel decouvert par une utilisatrice externe visitant le site deconnectee, jamais vu en developpant/testant toujours en session authentifiee.
...
**Mise en oeuvre (a rejouer) — defilement automatique du carrousel marques le 2026-09-09** : `brands_home` defile desormais tout seul (800ms), en plus des fleches/glisser qui restent pleinement fonctionnels **pendant** l'autoplay (cliquer une fleche ne doit ni rester sans effet ni interrompre durablement le defilement automatique). Desactive sous `prefers-reduced-motion`.

**Mise en oeuvre (a rejouer) — fondu discret a l'arrivee au scroll le 2026-09-09** ([ADR-049](../.claude/decisions/049-fondu-arrivee-scroll.md)) : le bloc solutions (`grid`/`grid_element`, section « Nos solutions auto-ecole et PMR ») et tout bloc `image_text_50`/`image_text_100` du site (ex. bloc configurateur de la home, pages solutions/corporate) revelent leur contenu (fondu + leger deplacement) en arrivant dans le viewport, a **chaque** passage (pas seulement au premier scroll). Sur `grid`, les cartes arrivent avec un **leger decalage vertical** entre elles (le fondu, lui, reste simultane sur toutes). Desactive sous `prefers-reduced-motion` ; sans JS, tout reste visible en permanence (pas de flash de contenu invisible).
...
- **Effet de zoom au survol du visuel (ajoute le 09/09/2026)** : sur `news-teaser` (liste) et `news-card` (bloc home), le visuel zoome legerement (scale 1.05, transition 0.3s) au survol de toute la carte/ligne — en plus, pas a la place, de la navigation deja en place. A rejouer en desktop (souris) uniquement, non pertinent au tactile.
...
- ✅ **Resolu le 2026-09-09** : le champ « Ajouter un document » avait 3 defauts visuels — (1) hauteur superieure aux autres champs (bouton natif « Choisir un fichier » desormais style, fond gris + curseur main) ; (2) le libelle passait a la ligne des **certaines resolutions desktop alors qu'il y avait de la place a droite** (le champ s'etale desormais sur 2 colonnes pour le libelle, le champ lui-meme restant a sa largeur normale d'une colonne) ; (3) **regression corrigee dans la meme session** : le correctif du (2) avait casse l'empilement mobile un champ par ligne (tous les champs de la section passaient en 2 colonnes implicites, quel que soit le choix Devis/SAV/Question). A rejouer sur les 3 largeurs (mobile, ~1024px, >=1280px) et sur les 3 choix du champ « Vous etes concerne par ».
...
- Rythme vertical du gabarit absent (aucun ecart titre→cartes, cartes→CTA,
  ...
  meme schema que `page-intro`/`news-list`/`.pager`. Verifie par mesure
  (`getComputedStyle`) : 49/64/49px desktop, 13/32/13px mobile.
- Bloc titre de page dedouble sur `/configurer` (et tout le configurateur) :
  cf. ADR-047 addendum — 6 `FormBase` du module `drivematic_configurator`
  posent deja leur propre `<h1>` manuel, desormais exclus du bloc partage
  (`/configurer` + `/configurer/*` ajoutes a la condition `request_path`).
- ⚠️ **Le lien de menu id 42 avait ete corrige en local mais pas en
  preprod** : les `menu_link_content` sont du contenu, jamais synchronise
  par `scripts/deploy-preprod.sh` (ADR-039) — a rejouer manuellement par
  SSH+drush sur chaque environnement apres toute modification de ce type
  (voir mémoire `menu-links-not-synced-to-preprod`).
...
- Le devis recoit le statut « a finaliser ». **Mise a jour du 2026-09-14** ([ADR-059](../.claude/decisions/059-reference-devis-a-la-commande.md)) : il **n'a pas encore** de numero `WAAAAMMJJ-001` a ce stade (colonne « N° devis » vide/masquee sur l'onglet « a finaliser ») — la reference n'est posee qu'a la commande (voir S16), pour rester coherente avec la date affichee ailleurs sur le devis.
...
**Pagination reelle + alignement de l'en-tete corriges — le 2026-09-14**
([ADR-060](../.claude/decisions/060-pagination-pdf-devis.md)) : le pied de
page affichait « Page 1/1 » identique sur **chaque** page d'un devis
multi-pages (rendu Twig en une seule passe HTML, avant toute pagination
Dompdf reelle). `QuotePdfGenerator::addPageNumbers()` dessine desormais
« Page X/Y » sur le canvas Dompdf apres `render()` ; « Adresse de
facturation : » aligne avec « 18 rue WOLFENBRUTTEL » (padding egal a la
hauteur du logo). Verifie sans creer de devis reel : un render array
`#theme: quote_pdf` fabrique (8 configurations, 4 pages), memes methodes
privees qu'en production invoquees par reflexion — PDF genere et lu
visuellement, en local **et** en preprod apres deploiement (memes resultats
...
2. Onglet « Commande en cours » : sur un devis « Commande en cours » (statut `a_commander`), verifier que **seul** Dupliquer est propose ; sur un devis « Commandé » (statut `commande`), verifier Dupliquer + Archiver + Telecharger le devis ; declencher l'archivage (modale de confirmation) et verifier que le devis bascule dans l'onglet « Archives ».
3. Onglet « Archives » (devis « Commande » + « Archive » reunis) : telecharger le PDF d'un devis, quel que soit son statut dans cet onglet ; verifier la colonne « N° devis ».
...
- L'archivage manuel n'est possible **que par le partenaire**, depuis l'onglet « Commande en cours » (pas depuis « Archives », qui n'a qu'un menu de telechargement) — Drive Matic n'a plus cette possibilite depuis le back-office (ADR-045).
- Colonne « Équipement(s) » : un `(x N)` apres chaque type d'equipement (quantite totale du devis, tous vehicules confondus), sur les **3 onglets**.
...
**Mise en oeuvre — etapes 2 et 3 (menu « Commande en cours »/« Archives »,
archivage manuel partenaire) livrees le 2026-09-14**
([ADR-057](../.claude/decisions/057-menu-actions-devis-en-cours.md),
remplace la ligne precedente qui les disait `[ ]`) :
- **Ecart assume par rapport a la maquette** (qui montrait aussi
  Commander/Modifier/Supprimer sur cet onglet, retires du perimetre par
  decision de l'utilisatrice) : le menu reellement livre sur « Commande en
  cours » est **Dupliquer seul** pour un devis `a_commander`, **Dupliquer +
  Archiver** pour un devis `commande` — jamais Commander/Modifier/Supprimer
  depuis cet onglet.
- « Archiver » (nouveau `QuoteArchiveForm`, `ConfirmFormBase` cote
  partenaire — distinct de l'ancien formulaire back-office du meme nom,
  supprime par ADR-045) : re-verifie cote serveur que le devis est bien
  `STATUS_COMMANDE`, le fait passer en `STATUS_ARCHIVE`, pose
  `date_archivage`, journalise une entree `quote_status_change` avec le
  partenaire comme auteur (jamais « Automatique », reserve au cron).
- « Telecharger le devis » (nouveau `QuotePdfDownloadController`, route
  `/user/mes-devis/{quote}/telecharger`, `Content-Disposition: attachment`)
  ajoute aux **deux** statuts de l'onglet « en cours », puis (addendum le
  meme jour) au seul menu de l'onglet « archivés » — une option, aucun
  nouveau code serveur, meme route reutilisee.
- Colonne « N° devis » etendue a l'onglet « archivés » (jusque-la reservee
  a « en cours »).
- Verifie via curl (partenaire reel, 5 devis « Commande en cours ») :
  lien de telechargement present sur les 5 lignes, PDF valide en-tete/
  xref/trailer ; archivage verifie de bout en bout (devis quitte « en
  cours », reapparait dans « archives », entree d'historique correcte).
...
**Correctif du 2026-09-07** : la grille du bloc « Votre entreprise » ne
reprenait pas encore l'agencement du webform `account_request` (meme jeu de
champs, meme ordre) — le champ TVA n'avait pas la classe `dm-form-span-2`
(risque de libelle sur 2 lignes). Aligne desormais a l'identique : Siret+TVA
(TVA sur 2 colonnes) / Raison sociale+Adresse+Complement / Code postal+Ville
(`dm-form-row-start`). Verifie par mesure des positions reelles (`getBoundingClientRect`),
...
|------|--------------|---------------------|
| 2026-09-15 | **2 regressions visuelles mobile de « Mes devis » corrigees** (SDC `quote-row`/`quote-row-actions`) : (1) la pastille de statut s'etirait a la hauteur complete de la ligne (touchant les filets separateurs) tout en restant etroite en largeur — largeur pleine colonne + hauteur bornee au contenu, centree, inset de 16px pour ne pas coller aux bords ; (2) le menu d'actions (3 points) se retrouvait recouvert par la carte du devis suivant a l'ouverture — `overflow: hidden` sur `.quote-row` (pose pour clipper les coins arrondis du fond gris de la colonne etiquette) rognait aussi ce menu, deja neutralise cote desktop mais pas mobile ; remplace par un `border-radius` porte directement par le pseudo-element | S18 |
| 2026-09-14 | **« Archiver » redirige vers l'onglet « Archives » au lieu de « Commande en cours »** (ADR-057, addendum) : demande explicite de l'utilisatrice, decision initiale reversee — le partenaire voit desormais directement ou son devis vient d'atterrir. `getCancelUrl()` (bouton « Non », etat degrade) reste sur « en cours » ; nouvelle `getArchivedUrl()` pour les 2 chemins de succes (non-AJAX et AJAX reel). Verifie via curl AJAX : reponse `{"command":"redirect","url":"/user/mes-devis?onglet=archives"}`, devis bien present sur cet onglet juste apres | S18 |
...
| 2026-09-14 | **Agregation `(x N)` par type d'equipement** dans la colonne « Equipement(s) » des 3 onglets de « Mes devis » (quantite totale du devis, tous vehicules confondus) — pas d'ADR dedie | S18 |
| 2026-09-14 | **Marque sans aucun modele publie exclue du select Marque du configurateur** ([ADR-058](../.claude/decisions/058-marques-orphelines-configurateur.md)) : la selectionner videait le select Modele sans message d'erreur (4 marques reelles concernees). Filtre en lecture (`ConfigurationForm`, intersection avec `modelsByBrand`), `vehicle_brand.status` jamais touche (webform contact/SAV non affecte) | S14 |
| 2026-09-14 | **Flash/decalage des boutons au clic corrige** (3 boutons : « Commander », « Ajouter une configuration », « Enregistrer le devis ») : Drupal core insere un `.ajax-progress` juste apres le bouton `#ajax` le temps de la requete, decalant ses freres dans une rangee `justify-content: flex-end` — masque via `.ajax-progress { display: none }` scope a chaque `.form-actions` concerne | S14, S16 |
| 2026-09-14 | **Civilite en minuscule corrigee dans les mails recus par DM** : `field_civility` est un `list_string`, `->value` renvoie la cle machine (`madame`/`monsieur`) jamais le libelle humain — resolu via `getFieldDefinition()->getSetting('allowed_values')` | S16 |
| 2026-09-14 | **Bouton « Voir le PDF du devis » recolle au lien « Retour a la liste des devis » sur la fiche devis back-office** : passe desormais a la ligne (conteneur `#type: container`) ; espacement rapproche du bouton « Marquer comme commande », ecarte du lien retour | S26 |
...
| 2026-09-09 | **Champ « Ajouter un document » du formulaire SAV corrige** : hauteur normalisee (bouton natif style, fond + curseur), libelle etale sur 2 colonnes en desktop (le champ garde sa largeur normale) — regression corrigee dans la meme session (l'empilement mobile un champ par ligne avait ete casse par le premier correctif) | S9 |
| 2026-09-09 | **Effet de zoom au survol des visuels d'actualite** (`news-card`, `news-teaser`, scale 1.05) et **libraires JS/CSS propres au theme** (`js-detect`/`slideshow`/`video-facade`/`reveal`/`global`) qui ne rechargeaient jamais apres modification (`version: VERSION` fige le cache-busting sur la version de Drupal core) — corrige | S2, S7, transverse |
...
| 2026-09-07 | **Correctif critique — `/admin/people/create` plantait systematiquement** (`InvalidQueryException`, `uid IN ()` vide) : `hook_form_user_form_alter` s'execute aussi sur `RegisterForm` (`base_form_id` partage avec `ProfileForm`), avec un compte pas encore enregistre — le recapitulatif des adresses de livraison (ADR-035) supposait a tort une entite existante. Corrige par une garde `$account->isNew()`. Bloquait la creation de compte partenaire en back-office (S22, ADR-025) depuis l'ajout de ce recapitulatif. Verifie (200, en local et en preprod, vraie session admin) ; `/user/register` verifie aussi (403, auto-inscription desactivee, jamais atteint par le bug). Piege verse au CLAUDE.md (section PHP/Drupal). **A rejouer** : S22 | S22 |
| 2026-09-07 | **Grille « Votre entreprise » de `PersonalInformationForm` alignee sur `account_request`** : `field_vat` n'avait pas la classe `dm-form-span-2` (risque de libelle sur 2 lignes) — ajoutee, avec `dm-form-row-start` sur `field_postal_code`, memes classes que le webform pour le meme jeu de champs dans le meme ordre. Verifie par mesure des positions reelles. **A rejouer** : S19 | S19 |
| 2026-09-07 | **Rythme vertical du tableau de bord** : aucun ecart de gabarit (titre→cartes, cartes→CTA, CTA→footer) n'etait applique. `--dm-space-page`/`--dm-space-block` ajoutes (ADR-013), meme schema que `page-intro`/`news-list`/`.pager`. Verifie par `getComputedStyle` : 49/64/49px desktop, 13/32/13px mobile. **A rejouer** : S13 | S13 |
| 2026-09-07 | **Double `<h1>` sur `/configurer` (et tout le configurateur)** : consequence du correctif du bloc titre de page (ADR-047) — 6 `FormBase` de `drivematic_configurator` posent deja leur propre titre manuel (texte parfois different du `_title` de la route, ex. les 3 ecrans du configurateur partagent le meme titre). Corrige par exclusion `request_path` (`/configurer` + `/configurer/*`) plutot que suppression des titres manuels (aurait change le texte affiche sur 4 des 6 formulaires). **A rejouer** : S14 | S14 |
| 2026-09-07 | **Telephone du footer rendu cliquable** (`tel:+33145342837`, meme traitement que le header) : seul le libelle « Tel : » reste un texte simple, le numero devient un vrai lien. **A rejouer** : Transverse | Transverse |
| 2026-09-07 | **⚠️ Lien de menu id 42 non synchronise en preprod** : corrige en local (ADR-046) mais les `menu_link_content` sont du contenu, jamais touche par `scripts/deploy-preprod.sh` (ADR-039) — reste en `<nolink>` sur la preprod malgre plusieurs deploiements du code portant la route. Rejoue manuellement par SSH+drush. Reflexe general ajoute en memoire (`menu-links-not-synced-to-preprod`) : verifier/repliquer explicitement toute modification de contenu (pas seulement code/config) sur chaque environnement | Hors matrice (deploiement) |
...
| 2026-09-03 | **Deploiement — sauvegarde preprod indisponible (mysqldump absent)** : `vendor/bin/drush sql:dump` (etape de securite de `scripts/deploy-preprod.sh` avant `config:import`) echoue systematiquement sur ce serveur — ni `mysql` ni `mysqldump` n'y sont installes (verifie par SSH), bien que PHP s'y connecte normalement a la base via `pdo_mysql` (base sur un hote separe). Pas un incident ponctuel : `--no-backup` est la seule option qui fonctionne tant que le client MySQL n'est pas installe sur ce serveur ou que le script n'est pas adapte pour dumper via PHP/PDO. 3 deploiements de la session (TVA, correctif grille webform, favicon) ont chacun ete termines avec `--no-backup`, sur confirmation explicite a chaque fois — aucun ne touchait a des donnees reelles au-dela d'un ajout de champ. Voir README.md, section Deploiement | Hors matrice (deploiement) |
| 2026-09-03 | **Favicon du site** (commit `6b8af67`) : jeu complet favicon.io (`favicon.ico`, 16×16/32×32, apple-touch-icon, 2 icones Android, `site.webmanifest`) dans `images/favicon/` du theme, balises posees dans `html.html.twig` via une nouvelle variable `favicon_path` (`drive_matic_preprocess_html()`). `favicon.ico` duplique en plus a la racine du theme pour que le mecanisme natif de Drupal (`system.theme.global`, `favicon.use_default`) le detecte automatiquement — sans ce doublon, Drupal serait retombe sur l'icone generique du cœur (`core/misc/favicon.ico`), le fichier attendu n'existant qu'en sous-dossier. Beneficie aussi au back-office (Gin), sans toucher a aucune config globale. Verifie : les 8 fichiers repondent en 200 (local et preprod), balises `<link>` presentes en DOM (JS), lint/format propres. **Deploye en preprod le meme jour** (`--no-backup`, meme raison que ci-dessus) | Transverse |
