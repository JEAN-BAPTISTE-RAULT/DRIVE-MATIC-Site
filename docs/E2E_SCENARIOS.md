...
- **Toute la ligne est cliquable depuis le 2026-08-21** (visuel, titre, date compris), pas seulement le lien « Lire la suite » — lien etire (`::before` sur `.news-teaser__more`, `.news-teaser` en `position: relative`), un seul `<a>` par ligne. Verifier un clic sur le visuel ou le titre, en desktop et mobile. `news-card` (bloc home) n'a **pas** recu le meme traitement.
...
- Seule l'**etape 1 (Configuration)** de ce scenario est aujourd'hui rejouable, sur `/configurer` (role `partenaire`). Les **etapes 2 et 3 (Devis, Livraison) ne sont pas implementees** — le bouton « Voir mon devis » soumet le formulaire et affiche un message de confirmation temporaire, sans tableau ni calcul de tarif.
