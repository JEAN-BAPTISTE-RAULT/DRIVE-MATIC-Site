...
- [ ] **Onglet « Archives »** : devis archives (manuellement [x] ou **auto a 30 jours** pour les commandes [x], `hook_cron`, ADR-033/038) ; dernier en tete ; **Telecharger le devis (PDF)** [x] (ADR-041 — genere au clic Commander, regenere sur remise DM, telechargeable depuis `/admin/content/devis/{id}` en back-office) ; un devis archive **ne peut plus etre duplique**. `[ ]` page « Mes devis » elle-meme non implementee (F13) : le PDF existe et se telecharge deja, reste a l'exposer sur cette page partenaire une fois creee.
