...
- [ ] **Documentations** : template dedie listant les documents Auto-ecole et PMR, ordre gere en back-office, chaque document affiche nom/format/poids (calcul auto).
  ...
  - **Structure d'un `document` — arbitre** : trois champs, tous obligatoires — un **titre administratif** (back-office : listes, recherche, autocompletion depuis la page Documentations ; jamais affiche en public), le **fichier**, et le **libelle du bouton de telechargement** (la description du fichier, [ADR-009](../.claude/decisions/009-telechargements-nommes.md)). Cette separation libelle d'administration / libelle public reste propre au type `document` : sur les types publics, l'[ADR-014](../.claude/decisions/014-titre-unique-porte-par-le-title.md) a ramene le titre a un seul champ, le `title`. Le rendu est un lien portant le libelle, suivi du **format et du poids calcules**.
