...
**Cas limites** :
- Item « Espace partenaire » : affiche le sous-menu authentifie (Tableau de bord, Mes devis, Mes informations personnelles, Me deconnecter) **uniquement** pour un partenaire connecte. « Supprimer mon compte » retire le 27/08 (voir Mise en oeuvre) ; reintegrera le menu quand le dernier critere d'acceptation de F12 sera implemente.
...
- **Mise en oeuvre — integration du 2026-08-25** ([ADR-024](../.claude/decisions/024-mutualisation-formulaire-simple.md)) : le node « Devenir partenaire » passe du bundle `partner` (retire) au bundle mutualise `simple_form`, qui porte desormais aussi « Demande de création de compte » (F12, page `/user/login`). Le partage temporaire initial du meme webform entre les deux nodes est termine le meme jour : « Demande de création de compte » a recu son propre webform dedie (`account_request`, voir F12).
...
- [x] L'adresse de **facturation** est affichee mais **non modifiable** en front (mise a jour via back-office uniquement). **Perimetre elargi a l'implementation** : tout le bloc « Votre entreprise » (Siret, TVA intracommunautaire, raison sociale, adresse, complement, code postal, ville) et l'e-mail sont lecture seule, pas seulement l'adresse — decision actee lors du plan (voir Mise en oeuvre ci-dessous).
...
- **Mise en oeuvre — integration du 2026-08-25** ([ADR-026](../.claude/decisions/026-profil-partenaire-mes-informations.md), maquette Figma 524:20069) : page « Mes informations personnelles » (`/user/mes-informations-personnelles`, module `drivematic_partner`), reservee au role `partenaire`. Reprend les champs du webform `account_request` ; seuls Civilite/Prenom/Nom/Fonction/Telephone sont modifiables, e-mail + bloc « Votre entreprise » en lecture seule. « Mot de passe perdu » redirige vers `/user/password` (formulaire core). Le formulaire core `/user/{uid}/edit` (partage avec le lien de definition de mot de passe de l'e-mail d'activation) est restreint en consequence : memes champs masques/lecture-seule, redirection vers « Mes informations personnelles » apres sauvegarde. « Supprimer mon compte » **reste non implemente** (dernier critere d'acceptation ci-dessus) ; son lien de menu (stub `<nolink>`) a ete retire le 27/08 en attendant l'implementation, plutot que de laisser un placeholder mort dans le menu.
- **Mise en oeuvre — integration du 2026-08-26** ([ADR-027](../.claude/decisions/027-confirmation-deconnexion.md)) : « Me deconnecter » mene desormais vers une page de confirmation (`/user/logout/confirm`, formulaire core) plutot qu'une deconnexion immediate. Un partenaire authentifie sur `/user/login` est redirige vers « Mes informations personnelles » plutot que sa page de compte par defaut (scope au role `partenaire`, meme raisonnement que la redirection post-sauvegarde de `/user/{uid}/edit`). Corrections responsive sur 3 pages du parcours : hauteur des 3 cartes d'action egalisee en mobile sur `/user/login` (egalisation JS, degradation gracieuse sans JS) ; boutons pleine largeur et alignes sur `/mes-informations-personnelles` (mobile et desktop) apres correction d'une marge fantome sur les `<input type="submit">` non stylises (`appearance: none` + `margin: 0`, generalisee a `_reset.scss` et `_forms.scss`) ; bouton « Valider » pleine largeur en mobile sur `/demande-de-creation-de-compte`.
- **Mise en oeuvre — integration du 2026-09-03** (commit `d02345c`) : numero de TVA intracommunautaire (`field_vat`, 13 caracteres) ajoute au bloc « Votre entreprise », meme traitement que les autres champs du bloc (obligatoire sur `account_request`, lecture seule sur « Mes informations personnelles » et masque sur `/user/{uid}/edit` en auto-edition). Repercute sur le devis (F15) : champ gele `Quote::billing_vat`, PDF et e-mail de confirmation de commande interne.
- **Correctif critique — le 2026-09-07** : `/admin/people/create` (creation d'un compte partenaire par Drive Matic, cf. ADR-025 ci-dessus) plantait systematiquement (`InvalidQueryException`, requete `uid IN ()` vide) — regression introduite en meme temps que le recapitulatif des adresses de livraison de la meme journee (ADR-035 ne l'avait pas anticipe : `hook_form_user_form_alter` s'execute aussi sur `RegisterForm`, qui partage le meme `base_form_id` que `ProfileForm`, avec un compte pas encore enregistre). Corrige par une garde explicite (`$account->isNew()`), verifie sur `/admin/people/create` **et** `/user/register` (403, auto-inscription publique desactivee — jamais atteint par le bug de toute facon), en local et en preprod. **Meme jour**, agencement de la grille « Votre entreprise » de « Mes informations personnelles » aligne sur celui du webform `account_request` (Siret+TVA / Raison sociale+Adresse+Complement / Code postal+Ville) — le champ TVA n'avait pas encore la classe de grille lui evitant un libelle sur 2 lignes. **Plus tard le meme jour**, champ « Alias d'URL » (base field core `path`, sans usage sur un compte utilisateur) masque a la creation, sur cette meme route, en reutilisant la garde `isNew()` ci-dessus.
...
**Criteres d'acceptation** :
- [x] Affiche : bouton « Creer un nouveau devis », nombre de devis a finaliser, nombre de devis et commandes en cours, visuel de valorisation du configurateur.

- **Mise en oeuvre — le 2026-09-07** (maquettes Figma 491-13703 desktop / 604-34427 mobile, [ADR-046](../.claude/decisions/046-tableau-de-bord-partenaire.md)) : route `/user/tableau-de-bord` (module `drivematic_partner`), 3 compteurs scopes au partenaire connecte (jamais un uid transmis par le client) — « À finaliser » seul, « Commande en cours » seul, « Commande »+« Archive » **reunis** dans un seul compteur/onglet « commandes archivees » (decision actee cette session, a reprendre telle quelle pour la structure des onglets de la page « Mes devis » ci-dessous). Les 3 compteurs redirigent vers `/user/mes-devis?onglet=...` (page listee ci-dessous, **hors perimetre de cette etape**) : lien pose en dur, la route n'existant pas encore. Lien de menu « Tableau de bord » (existait deja en `<nolink>`) rendu fonctionnel.
...
**Criteres d'acceptation** :
- [x] Le tarif remise applique = remise commerciale du partenaire, **par equipement** (4 taux independants, ADR-043, 2026-09-03 — remplace l'ancien taux unique `field_discount_rate`), fige a la creation du devis (aucun suivi en direct des changements ulterieurs du compte). [x] Remise « exceptionnelle » saisie par Drive Matic pour ce devis precis, regroupee par equipement plutot que par ligne — voir F15.
...
- [x] Chaque configuration est modifiable / supprimable dans l'etape Devis (suppression avec modale de confirmation depuis le 2026-09-01, `QuoteConfigurationDeleteForm`).
...
- [x] **Ajoute le 2026-09-14** ([ADR-058](../.claude/decisions/058-marques-orphelines-configurateur.md)) : une marque sans **aucun** modele publie (tous depublies au dernier import catalogue, F17) est exclue du `<select>` Marque plutot que de mener a un `<select>` Modele vide sans message d'erreur.
...
- **Suite le 2026-09-01 (meme jour)** : persistance verifiee de bout en bout par un parcours navigateur reel (Configuration → Devis → Livraison → « Commander »). Suppression d'une configuration a l'ecran Devis dotee d'une modale de confirmation (`QuoteConfigurationDeleteForm`, meme mecanisme qu'ADR-034, remplace une suppression jusque-la immediate et sans confirmation). Un devis enregistre n'etant consultable nulle part (pas meme en back-office), ajout d'un listing admin `/admin/content/devis` (Vue Drupal, tri par colonne, filtre Statut, recherche par reference — permission `view drivematic configurator quotes`) et d'un recapitulatif en lecture seule des adresses de livraison d'un partenaire sur `/user/{uid}/edit` cote admin ([ADR-035](../.claude/decisions/035-recap-adresses-livraison-admin.md)). Les 3 modales (ajout/edition/suppression d'adresse) realignees au pixel pres sur la maquette 521:17375 (addendum ADR-034).
...
**Criteres d'acceptation** :
- [x] Numerotation devis : `WAAAAMMJJ-001` (compteur journalier, `QuoteReferenceGenerator`, ADR-033). **Mise a jour du 2026-09-14** ([ADR-059](../.claude/decisions/059-reference-devis-a-la-commande.md)) : la reference est posee **a la commande** (transition vers `STATUS_A_COMMANDER`), jamais a la simple creation d'un brouillon « a finaliser » — un devis « a finaliser » n'a donc **aucune** reference tant qu'il n'est pas commande (coherent avec la colonne N° devis, deja masquee sur cet onglet, cf. ligne suivante).
- [x] **Onglet « A finaliser »** : devis non commandes ; dernier en tete ; colonnes date / marque / modele / type / equipements / statut ; pagination 10 ; fonctions **Modifier, Dupliquer, Supprimer** (avec confirmation). `[ ]` page « Mes devis » elle-meme non implementee (hors perimetre F14 3/3, cf. ADR-033). **Mise a jour du 2026-09-09** ([ADR-051](../.claude/decisions/051-page-mes-devis-listing.md)/[ADR-052](../.claude/decisions/052-menu-actions-devis-a-finaliser.md)) : page « Mes devis » (`/user/mes-devis`, 3 onglets) construite ; colonnes/tri reordonnes par rapport a la description initiale ci-dessus pour correspondre aux maquettes reelles (voir ADR-051). Les 3 actions sont operationnelles sur cet onglet : Modifier redirige vers l'ecran Livraison du configurateur pour resauvegarder le devis **en place** (meme id/reference) ; Dupliquer cree un nouveau devis « a finaliser » ; les deux recalculent systematiquement au tarif catalogue et a la remise partenaire du jour, un devis « a finaliser » n'etant pas fige au sens F15 ci-dessous (precision ADR-052 sur ADR-043). **Mise a jour du 2026-09-11** (ADR-056) : ce lien est renomme « Reprendre » et ouvre desormais une modale d'avertissement (catalogue susceptible d'avoir change) avant de rediriger vers l'**etape 1** du configurateur (plus l'etape 3 comme avant) ; « Enregistrer le devis » (etape 3) n'affiche plus de message et redirige directement vers « Mes devis », onglet « à finaliser ». **Mise a jour du 2026-09-14** : la colonne « equipements » des **3 onglets** de « Mes devis » affiche desormais une agregation `(x N)` par type d'equipement apres son libelle (quantite totale, tous vehicules confondus du devis) plutot que le seul libelle repete par ligne.
- [x] **Onglet « Commande en cours »** : devis finalises non commandes ; ajoute colonnes n° devis + montant HT ; statuts « Commande en cours » ou « Commande le jj/mm/aaaa ». **Mise a jour du 2026-09-02** [x] : le statut a 4 valeurs decrit ici est desormais implemente (`Quote::STATUS_A_FINALISER`/`STATUS_A_COMMANDER`/`STATUS_COMMANDE`/`STATUS_ARCHIVE`, ADR-038) — reste hors perimetre uniquement la page « Mes devis »/onglets elle-meme (F13), qui affichera ces statuts au partenaire. **Mise a jour du 2026-09-07** ([ADR-045](../.claude/decisions/045-archivage-devis-depuis-commande.md)) : le statut `a_commander` est renomme « Commande en cours » (etait « A commander ») ; un devis « Commande » ne figure **plus** dans cet onglet (voir onglet « Archives » ci-dessous, qui le regroupe desormais avec les devis archives — decision actee avec le tableau de bord, F13/ADR-046).
  - Devis non commande : **Commander** (renvoi a l'etape Devis du configurateur), Modifier, Dupliquer, Supprimer. **Mise a jour du 2026-09-09** (ADR-052) : menu de cet onglet explicitement **hors perimetre** de la livraison du 2026-09-09, qui n'a couvert que l'onglet « A finaliser » ci-dessus — reste `[ ]` a construire. **Mise a jour du 2026-09-14** ([ADR-057](../.claude/decisions/057-menu-actions-devis-en-cours.md), remplace la ligne precedente) [x] : ecart assume par rapport a la maquette (qui montrait aussi Commander/Modifier/Supprimer, retires du perimetre par decision de l'utilisatrice) — le menu reellement livre est **Dupliquer seul** pour un devis « Commande en cours », **Dupliquer + Archiver** pour un devis « Commande », et **Telecharger le devis** (PDF) pour les deux statuts. « Archiver » (nouveau `QuoteArchiveForm`, cote partenaire — distinct de l'ancien formulaire back-office du meme nom, supprime par ADR-045) fait passer le devis en « Archive » et l'implante l'archivage manuel partenaire annonce a la ligne suivante.
- [x] **Commander** : enregistre la date de commande [x] (`Quote::date_commande`), affiche « Felicitations, votre commande a bien ete enregistree... » [x] (DeliveryForm), envoie l'e-mail de confirmation au partenaire et une copie interne (`info@drivematiclegrand.com`, adresse temporaire `audrey@passerelle.com` en attendant la mise en prod) [x] (ADR-036, `hook_mail()` + Mailer Policy), **avec le PDF du devis en piece jointe** [x] (ADR-041, `dompdf`, genere au clic Commander). **Ajoute le 2026-09-11** ([ADR-056](../.claude/decisions/056-verification-catalogue-devis.md)) [x] : verification finale du catalogue au clic (prix/disponibilite recalcules, compares a l'instantane pris en quittant l'ecran Devis) — si un changement reel est detecte, rien n'est persiste et une modale d'avertissement s'affiche (« Poursuivre la commande » finalise avec les valeurs a jour, « Annuler » retourne a l'etape 1) ; sinon la commande se finalise normalement, sans aucune modale. **Mise a jour du 2026-09-14** : redirige vers « Mes devis », onglet « Commande en cours » (le devis vient d'y apparaitre) — avant : etape 1 du configurateur.
- [x] **Onglet « Archives »** : devis « Commande » **et** devis « Archive » (regroupes dans le meme onglet depuis le 2026-09-07, F13/ADR-046) ; **auto a 30 jours pour les devis « Commande »** [x] (`hook_cron`, ADR-045) ; dernier en tete ; **Telecharger le devis (PDF)** [x] (ADR-041 — genere au clic Commander, regenere sur remise DM, telechargeable depuis `/admin/content/devis/{id}` en back-office) ; un devis archive **ne peut plus etre duplique**. Archivage manuel reserve **au partenaire seul**, depuis cette page (a construire) — Drive Matic n'a plus cette possibilite depuis le back-office (voir ligne suivante). `[ ]` page « Mes devis » elle-meme non implementee (F13) : le PDF existe et se telecharge deja, reste a l'exposer sur cette page partenaire une fois creee. **Mise a jour du 2026-09-09** (ADR-051/052) : la page « Mes devis » et son onglet « Archives » existent desormais (listing seul, sans menu d'actions — hors perimetre ADR-052) ; le PDF n'y est pas encore expose au partenaire ; **reste `[ ]`** : archivage manuel partenaire, exposition du PDF sur cet onglet. **Mise a jour du 2026-09-14** (ADR-057, remplace le `[ ]` precedent) [x] : menu 3 points a une seule option, **Telecharger le devis**, ajoute sur cet onglet (meme route que l'onglet « en cours ») ; colonne **N° devis** etendue a cet onglet (jusque-la reservee a « en cours »). Archivage manuel partenaire livre a la ligne precedente (via l'onglet « en cours », action « Archiver » sur un devis « Commande » — le devis bascule alors dans cet onglet « Archives »).
- [x] **Mise a jour du 2026-09-07** ([ADR-045](../.claude/decisions/045-archivage-devis-depuis-commande.md), remplace la ligne precedente) : Drive Matic n'a plus la possibilite d'archiver un devis manuellement depuis le back-office, quel que soit son statut (`QuoteArchiveForm` supprime). Seul le partenaire pourra le faire, pour un devis au statut « Commande », depuis son tableau de bord (page « Mes devis », a construire). L'archivage automatique part desormais de « Commande » (30 jours depuis `date_confirmation`, delai fixe, sans mecanisme de report) et non plus de « Commande en cours » : un devis qui reste indefiniment « Commande en cours » sans confirmation telephonique de Drive Matic n'a plus de mecanisme de nettoyage automatique (consequence connue, actee avec l'utilisatrice).

**Cas limites** :
- [x] Remise supplementaire : le partenaire appelle Drive Matic ; DM saisit une remise **par equipement** (section « Remises par équipement », 4 lignes fixes plutot qu'une par ligne/configuration, ADR-043 addendum) en back-office tant que le devis n'a **pas** ete commande (statut « Commande en cours », renomme le 2026-09-07, ADR-045) — visible en **lecture seule** sur tout autre statut, jamais editable. Cette remise **remplace** (jamais cumulee) le taux partenaire fige sur le devis a sa creation, calculee depuis le tarif catalogue brut ; le compte partenaire n'est jamais modifie par cette action. **Mise a jour du 2026-09-07** (ADR-045) : n'enregistre plus de reinitialisation d'un compteur de 30 jours — ce mecanisme etait uniquement au service de l'ancienne regle d'archivage automatique (depuis « Commande en cours »), remplacee par une regle qui part desormais de « Commande » (voir plus bas), sur laquelle cette remise ne peut de toute facon plus s'appliquer. **Tracee dans l'historique du devis** (qui a accorde quelle remise, sur quelle ligne, et quand — entite `quote_discount_change`, ADR-040) et dans l'**historique du compte partenaire** (qui a change quel taux, quand — entite `partner_discount_change`, ADR-044, visible sur `/user/{uid}/edit`) : ce dernier permet de justifier un ecart entre le taux affiche aujourd'hui sur le compte et celui reellement fige sur un devis cree avant ce changement.
- [x] Devis commande depuis > 30 jours → archivage automatique (`drivematic_configurator_cron()`, verifie J+29 vs J+31). **Mise a jour du 2026-09-07** (ADR-045, remplace la version precedente de cette ligne qui excluait justement le statut « Commande ») : c'est desormais l'inverse — la regle s'applique **uniquement** a un devis au statut « Commande » (30 jours depuis `date_confirmation`), plus jamais a un devis « Commande en cours ».
- [x] **Ajoute le 2026-09-10** ([ADR-053](../.claude/decisions/053-purge-pdf-devis.md)) : le PDF d'un devis dont `date_confirmation` depasse 2 ans est supprime automatiquement (`hook_cron()`, meme date de reference que l'archivage ci-dessus) — irreversible, seul le fichier disparait, l'entite Quote et son historique restent intacts.
...
- [x] Gestion des conditions commerciales : **4 taux de remise par equipement** par partenaire (`field_discount_retrovision_ext`/`retrovision_int`/`telecommande_vor`/`pedalier`, ADR-043, 2026-09-03 — remplace l'ancien taux unique `field_discount_rate` de l'ADR-026 ; modifiables uniquement en back-office, jamais exposes au partenaire) + **remise exceptionnelle par equipement** sur un devis donne [x] (`QuoteEquipmentLine::dm_discount_rate` + section « Remises par équipement », ADR-038/043 — editable tant que le devis est au statut « A commander », consultable en lecture seule sinon) + **historique des changements de remise** par compte partenaire [x] (`partner_discount_change`, ADR-044, `/user/{uid}/edit`).
...
- [x] Statut du combinatoire (« À publier »/« Ne pas publier ») pilote la publication du vehicule dans le configurateur ; un modele « à publier » sans tarif pedalier bloque l'import (ADR-055).
...
**Constat du 2026-09-03** : le champ `reference` du catalogue est **vide pour rétrovision ext./int. et télécommande VOR** (139 lignes au total, faute de donnee dans le fichier source) — seul le pedalier (274 lignes) en a systematiquement une. Anticipe des la conception (`docs/plans/catalogue-tarifs-import.md`), pas une regression de l'import : la colonne « Référence » du detail de devis (ADR-041/043) affiche un tiret pour ces 3 equipements tant que le fichier source n'en fournit pas.

**Mise a jour du 2026-09-11** ([ADR-055](../.claude/decisions/055-statut-publication-vehicule-type-vor.md)/[ADR-056](../.claude/decisions/056-verification-catalogue-devis.md)) : la colonne **Statut** du combinatoire (« À publier sur le site » / « Ne pas publier »), jusque-la lue puis ignoree, pilote desormais la publication du terme `vehicle_model` (depublie, jamais supprime — les listes de selection du configurateur/formulaire de contact ne proposent que les modeles publies ; un vehicule depublie referme par un vieux devis est traite comme un tarif indisponible, meme mecanisme que pour un produit retire du catalogue). La colonne **Type de VOR** est desormais stockee (`equipment_price.type_vor`, visible dans le catalogue admin). **Nouvelle regle bloquante** : un modele marque « À publier sur le site » sans aucun tarif pedalier pour aucune motorisation **empeche desormais tout l'import** (erreur listant tous les modeles concernes), plutot que d'etre exclu silencieusement du catalogue comme c'etait le cas depuis la conception. Reprendre un devis « à finaliser » (menu « Mes devis ») affiche desormais une modale d'avertissement avant de rediriger vers l'etape 1 du configurateur (plus l'etape 3) ; Commander effectue une verification finale du catalogue et n'avertit que si un changement reel est detecte depuis l'ecran Devis.
...
| **Devis** | N° `WAAAAMMJJ-001`, statut (`a finaliser` / `commande en cours` / `commande le jj/mm/aaaa`), date de creation, date de commande, archive (bool) + date, totaux (HT, remise HT, TVA 20 %, TTC) | N → 1 partenaire ; 1 → 1..10 configurations ; N → 1 adresse de livraison |
...
| **Ligne d'equipement** | Produit, reference catalogue figee (ADR-041, vide si non disponible a la source), quantite par vehicule, quantite totale, tarif catalogue HT, taux de remise fige a la creation (par equipement, remplacable par une remise Drive Matic sur ce devis, ADR-043) | N → 1 configuration ; N → 1 produit |
...
- Le tarif remise d'une ligne = taux partenaire par equipement **fige a la creation du devis** (jamais de suivi en direct d'un changement ulterieur du compte, ADR-043) — **remplace**, jamais cumule, par une remise exceptionnelle Drive Matic saisie explicitement sur ce devis (n'altere jamais le compte partenaire).
...
Couleurs de statut des devis (cf. F15) : rouge « a finaliser », orange « commande en cours », vert « commande ».
