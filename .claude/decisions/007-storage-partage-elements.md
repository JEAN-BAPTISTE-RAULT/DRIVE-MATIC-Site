...
- V5 : `product_characteristics` **reutilise `field_elements`** (illimite, instance restreinte a `product_characteristic_element`). Les blocs plafonnes recoivent leur storage dedie : **`field_features_elements`** (cardinalite `5`, instance `product_features` ciblant **deux** bundles Element — `product_image_element` + `product_video_element`) et **`field_cross_elements`** (cardinalite `5`, instance `product_cross` ciblant `product_cross_element`). Les 4 Element produit sont exclus du placement direct. (Cas hors ERR, mentionne pour trace : `product_arguments` = simple champ string plafonne `field_arguments` card 3, pas de paire Bloc/Element.)
